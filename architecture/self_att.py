...
class BimodalSelfAttention(nn.Module):
    ...
    def _cross_attention_with_noise(self, q_input, keys_input, Wq, Wk, Wg, Wm, out_proj, noise_param, c, keys_mask=None):
        """
        q_input: (B, T_q, D)  queries (already the self-attended queries of source modality)
        keys_input: (B, T_k, D) keys/values (raw other-modality self-attended? here we use original modality)
        returns: (B, T_q, D) projected cross-attended features with residual from keys_input (aligned or pooled)
        """
